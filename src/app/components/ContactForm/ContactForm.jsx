...
const formSchema = z.object({
  ...
  user_phone: z.string()
    ...
    .regex(/^3\d{9}$/, "Debe ser un número de celular colombiano válido (ej: 3123456789)"),
  user_email: z.string()
      ...
      <div className={style.contact__form__container}>
        ...
        >
          {submitted ? (
            ...
            <form ref={formRef} onSubmit={validateSubmit(handleFormSubmit)} noValidate>
              <h2>Llámanos o escríbenos</h2>
              
              <div className={`${style.input_group} ${watchedFields.user_name ? style.has_value : ''}`}>
                <label htmlFor="user_name" className={style.floating_label}>Nombre</label>
                <input 
                  id="user_name"
                  type="text" 
                  placeholder="Nombre" 
                  className={errors.user_name && (touchedFields.user_name || attemptedSubmit) ? style.error_input : ""}
                  {...register("user_name")} 
                  data-tooltip={errors.user_name && (touchedFields.user_name || attemptedSubmit) ? errors.user_name.message : ""}
                />
              </div>
              
              <div className={`${style.input_group} ${watchedFields.user_phone ? style.has_value : ''}`}>
                <label htmlFor="user_phone" className={style.floating_label}>Celular</label>
                <input 
                  id="user_phone"
                  type="tel" 
                  inputMode="numeric"
                  ...
                  maxLength={10}
                  className={errors.user_phone && (touchedFields.user_phone || attemptedSubmit) ? style.error_input : ""}
                  {...register("user_phone")} 
                  onInput={handlePhoneInput}
                  data-tooltip={errors.user_phone && (touchedFields.user_phone || attemptedSubmit) ? errors.user_phone.message : ""}
                />
              </div>
              
              <div className={`${style.input_group} ${watchedFields.user_email ? style.has_value : ''}`}>
                <label htmlFor="user_email" className={style.floating_label}>Correo</label>
                <input 
                  id="user_email"
                  type="email" 
                  placeholder="Correo" 
                  className={errors.user_email && (touchedFields.user_email || attemptedSubmit) ? style.error_input : ""}
                  {...register("user_email")} 
                  data-tooltip={errors.user_email && (touchedFields.user_email || attemptedSubmit) ? errors.user_email.message : ""}
                />
              </div>
              
              <div className={`${style.input_group} ${watchedFields.message ? style.has_value : ''}`}>
                <label htmlFor="message" className={style.floating_label}>Mensaje</label>
                <textarea 
                  id="message"
                  ref={messageRef}
                  placeholder="En qué te podemos ayudar (mínimo 10 caracteres)" 
                  className={`${style.textarea} ${errors.message && (touchedFields.message || attemptedSubmit) ? style.error_input : ""}`}
                  {...register("message")} 
                  data-tooltip={errors.message && (touchedFields.message || attemptedSubmit) ? errors.message.message : ""}
                  rows={3}
                ></textarea>
              </div>
